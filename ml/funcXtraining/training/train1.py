...
def main(args, model):
    ...
    from training.train1 import plot_input
    #from data import *
    import re
...
def test(input_file, output_path, config, example_run=False, model='GANv1'):
    ...
    parser.add_argument('-o', '--output_path', type=str, required=False, default='../output/dataset1/v1', help='Training h5 file path (default: %(default)s)')
...
def main1(input_file, output_file="output.tar.gz", example_run=False, config="./config/config_GANv1.json", model='GANv1'):
    import os
    import tarfile
    from argparse import ArgumentParser

    all_models = [ 'GANv1' ]
    parser = ArgumentParser(description="Config for training")
    parser.add_argument('-i', '--input_file', type=str, required=False, default='', help='Training h5 file name (default: %(default)s)')
    parser.add_argument('-o', '--output_path', type=str, required=False, default='../output/dataset1/v1', help='Training h5 file path (default: %(default)s)')
    parser.add_argument('-c', '--config', type=str, required=False, default=None, help='External config file (default: %(default)s)')
    parser.add_argument('-m', '--models', nargs='*', required=False, default=all_models, choices=all_models, help='Models to train (default: %(default)s)')
    ...
    args.input_file = input_file
    args.output_path = "./output_files"
    if config:
        args.config = config
    if example_run:
        args.example_run = True

    main(args, model)

    with tarfile.open(output_file, "w:gz", dereference=True) as tar:
        tar.add(args.output_path,arcname=os.path.basename(args.output_path))
